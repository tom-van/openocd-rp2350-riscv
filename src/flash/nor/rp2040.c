...
COMMAND_HANDLER(rp2040_switch_target_handler)
{
	...
	if (!old_target) {
		LOG_ERROR("Unrecognised old target %s", CMD_ARGV[0]);
		return ERROR_FAIL;
	}

	struct target *new_target = get_target(CMD_ARGV[1]);
	if (!new_target) {
		LOG_ERROR("Unrecognised new target %s", CMD_ARGV[1]);
		return ERROR_FAIL;
	}
	...
	LOG_ERROR("Neither old nor new target %s found in flash bank list", CMD_ARGV[0]);
	return ERROR_FAIL;
}
